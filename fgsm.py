...
class aaeCaptcha:
    ...
    def fgsm(self, audios, lengths, phrases, phrase_lens, epsilon=350.0):
        sess = self.sess
        ...
        grad_sign_np = sess.run(grad_sign, feed_dict=feed_dict)
        adv = audios + epsilon * grad_sign_np
        return adv
        ...
        with tf.Session(config=config) as sess:
            ...
            for l in range(num_loops):
                ...
                for i in range(adv.shape[0]):
                    ...
                    path = "{}_fgsm_{}.wav".format(path, int(args.epsilon))
